...
do_compile() {
	setup_build_variables
	make -C "${LINUX_DIR}" CROSS_COMPILE="${TARGET_CROSS}" M="${S}" modules
}
...
do_install() {
	...
	cp ${TOPDIR}/../opensource/qca-emesh-sp/*.h  ${STAGING_DIR}/usr/include/
	install -d ${D}${nonarch_base_libdir}/modules/${KERNEL_VERSION}/extra
	install -m 0755 ${S}/*.ko ${D}/${nonarch_base_libdir}/modules/${KERNEL_VERSION}/extra/
	install -m 0755 ${S}/Module.symvers ${D}/usr/include/emesh-sp/
